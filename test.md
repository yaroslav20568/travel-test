...
### 1. Store корзины (Zustand)

#### Тест-кейсы для `useCartStore`
...
- **TC-CART-002**: Удаление товара из корзины
  - Проверка удаления существующего товара
  - Проверка удаления несуществующего товара
  - Проверка обновления состояния после удаления

- **TC-CART-003**: Очистка корзины
  - Проверка полной очистки корзины
  - Проверка обновления состояния

- **TC-CART-004**: Расчет общей стоимости
  - Проверка расчета суммы для пустой корзины
  - Проверка расчета суммы для корзины с товарами
  - Проверка корректности суммирования цен

- **TC-CART-005**: Управление состоянием открытия/закрытия
  - Проверка открытия корзины
  - Проверка закрытия корзины
...
#### Тест-кейсы для `useIsMobile`

- **TC-HOOK-005**: Определение мобильного устройства
  - Проверка определения мобильного устройства при ширине <= breakpoint
  - Проверка определения десктопа при ширине > breakpoint
  - Проверка реакции на изменение размера окна
  - Проверка кастомного breakpoint

#### Тест-кейсы для `useBodyScrollLock`

- **TC-HOOK-006**: Блокировка скролла
  - Проверка блокировки скролла при `isLocked = true`
  - Проверка разблокировки скролла при `isLocked = false`
...
- Store корзины (TC-CART-001 - TC-CART-005)
- Утилита форматирования цены (TC-UTIL-001)
- Компоненты корзины (TC-UI-004 - TC-UI-014)
- Интеграционные тесты основных потоков (TC-INT-001, TC-INT-002)

### Средний приоритет

- API сервисов (TC-API-001 - TC-API-004)
...
- Хуки для определения устройства и блокировки скролла (TC-HOOK-005 - TC-HOOK-007)
